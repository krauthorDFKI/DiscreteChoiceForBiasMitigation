...
###############################
    ...
    def _prepare_model(self):
        with tf.name_scope("input_data"):
            ...
            self.item_input = tf.compat.v1.placeholder(tf.int32, shape=[None, 1], name="item_input")
            self.label_input = tf.compat.v1.placeholder(tf.float32, shape=[None, 1], name="label_input")
            self.pop_input = tf.compat.v1.placeholder(tf.float32, shape=[None, 1], name="pop_input")
            self.rel_input = tf.compat.v1.placeholder(tf.float32, shape=[None, 1], name="rel_input")
            self.exp_input = tf.compat.v1.placeholder(tf.float32, shape=[None, 1], name="exp_input")
        ...
        for m in range(self.C):
            ...
            with tf.compat.v1.variable_scope('Relevance_' + str(m), reuse=tf.AUTO_REUSE):
                P = tf.compat.v1.get_variable(name='P_' + str(m),
                                    initializer=tf.truncated_normal(shape=[self.num_user, self.hidden],
                                                                    mean=0, stddev=0.03,
                                                                    dtype=tf.float32), dtype=tf.float32)
                Q = tf.compat.v1.get_variable(name='Q_' + str(m),
                                    initializer=tf.truncated_normal(shape=[self.num_item, self.hidden],
                                                                    mean=0, stddev=0.03,
                                                                    dtype=tf.float32), dtype=tf.float32)
            para_rel = tf.compat.v1.get_collection(tf.GraphKeys.GLOBAL_VARIABLES, scope='Relevance_' + str(m))
            self.P_list.append(P)
            self.Q_list.append(Q)
            self.para_rel_list.append(para_rel)

            with tf.compat.v1.variable_scope('Exposure_' + str(m), reuse=tf.AUTO_REUSE): 
                c = tf.compat.v1.get_variable(name='c_' + str(m),
                                    initializer=tf.truncated_normal(shape=[self.hidden, 1],
                                                                    mean=0, stddev=0.03,
                                                                    dtype=tf.float32), dtype=tf.float32)
                d = tf.compat.v1.get_variable(name='d_' + str(m),
                                    initializer=tf.truncated_normal(shape=[1, 1],
                                                                    mean=0, stddev=0.03,
                                                                    dtype=tf.float32), dtype=tf.float32)
                a = tf.compat.v1.get_variable(name='a_' + str(m),
                                    initializer=tf.truncated_normal(shape=[self.hidden, 1],
                                                                    mean=0, stddev=0.03,
                                                                    dtype=tf.float32), dtype=tf.float32)
                b = tf.compat.v1.get_variable(name='b_' + str(m),
                                    initializer=tf.truncated_normal(shape=[1, 1],
                                                                    mean=0, stddev=0.03,
                                                                    dtype=tf.float32), dtype=tf.float32)
                e = tf.compat.v1.get_variable(name='e_' + str(m),
                                    initializer=tf.truncated_normal(shape=[self.hidden, 1],
                                                                    mean=0, stddev=0.03,
                                                                    dtype=tf.float32), dtype=tf.float32)
                f = tf.compat.v1.get_variable(name='f_' + str(m),
                                    initializer=tf.truncated_normal(shape=[1, 1],
                                                                    mean=0, stddev=0.03,
                                                                    dtype=tf.float32), dtype=tf.float32)
            para_exp = tf.compat.v1.get_collection(tf.GraphKeys.GLOBAL_VARIABLES, scope='Exposure_' + str(m))
        ...
        for m in range(self.C):
            ...
            rel_cost = -tf.reduce_mean(self.label_input / self.exp_input * tf.compat.v1.log(rel_predict)
                                       + (1 - self.label_input / self.exp_input) * tf.compat.v1.log(1 - rel_predict))
            exp_cost = -tf.reduce_mean(self.label_input / self.rel_input * tf.compat.v1.log(exp_predict)
                                       + (1 - self.label_input / self.rel_input) * tf.compat.v1.log(1 - exp_predict))
        ...
        for m in range(self.C):
            tf.compat.v1.set_random_seed(self.seed)
            with tf.compat.v1.variable_scope('rRelevance_' + str(m), reuse=tf.AUTO_REUSE):
                P = tf.compat.v1.get_variable(name='rP_' + str(m),
                                    initializer=tf.truncated_normal(shape=[self.num_user, self.hidden],
                                                                    mean=0, stddev=0.01,
                                                                    dtype=tf.float32), dtype=tf.float32)
                Q = tf.compat.v1.get_variable(name='rQ_' + str(m),
                                    initializer=tf.truncated_normal(shape=[self.num_item, self.hidden],
                                                                    mean=0, stddev=0.01,
                                                                    dtype=tf.float32), dtype=tf.float32)
            rpara_rel = tf.compat.v1.get_collection(tf.GraphKeys.GLOBAL_VARIABLES, scope='rRelevance_' + str(m))
            self.rP_list.append(P)
            self.rQ_list.append(Q)
            self.rpara_rel_list.append(rpara_rel)
            with tf.compat.v1.variable_scope('rExposure_' + str(m), reuse=tf.AUTO_REUSE):
                c = tf.compat.v1.get_variable(name='rc_' + str(m),
                                    initializer=tf.truncated_normal(shape=[self.hidden, 1],
                                                                    mean=0, stddev=0.01,
                                                                    dtype=tf.float32), dtype=tf.float32)
                d = tf.compat.v1.get_variable(name='rd_' + str(m),
                                    initializer=tf.truncated_normal(shape=[1, 1],
                                                                    mean=0, stddev=0.01,
                                                                    dtype=tf.float32), dtype=tf.float32)
                a = tf.compat.v1.get_variable(name='ra_' + str(m),
                                    initializer=tf.truncated_normal(shape=[self.hidden, 1],
                                                                    mean=0, stddev=0.01,
                                                                    dtype=tf.float32), dtype=tf.float32)
                b = tf.compat.v1.get_variable(name='rb_' + str(m),
                                    initializer=tf.truncated_normal(shape=[1, 1],
                                                                    mean=0, stddev=0.01,
                                                                    dtype=tf.float32), dtype=tf.float32)
                e = tf.compat.v1.get_variable(name='re_' + str(m),
                                    initializer=tf.truncated_normal(shape=[self.hidden, 1],
                                                                    mean=0, stddev=0.01,
                                                                    dtype=tf.float32), dtype=tf.float32)
                f = tf.compat.v1.get_variable(name='rf_' + str(m),
                                    initializer=tf.truncated_normal(shape=[1, 1],
                                                                    mean=0, stddev=0.01,
                                                                    dtype=tf.float32), dtype=tf.float32)
            rpara_exp = tf.compat.v1.get_collection(tf.GraphKeys.GLOBAL_VARIABLES, scope='rExposure_' + str(m))
        ...
        for m in range(self.C):
            ...
            rrel_cost = -tf.reduce_mean(self.label_input * tf.compat.v1.log(rel_predict * self.exp_input)
                                        + (1 - self.label_input) * tf.compat.v1.log(1 - rel_predict * self.exp_input))
            rexp_cost = -tf.reduce_mean(self.label_input * tf.compat.v1.log(self.rel_input * exp_predict)
                                        + (1 - self.label_input) * tf.compat.v1.log(1 - self.rel_input * exp_predict))
        ...
        with tf.compat.v1.variable_scope("Optimizer", reuse=tf.AUTO_REUSE):
            for m in range(self.C):
                rel_optimizer = tf.compat.v1.train.AdamOptimizer(learning_rate=self.lr).minimize(self.rel_cost_final_list[m],
                                                                                       var_list=self.para_rel_list[m]) # Previously tf.train.AdamOptimizer
                exp_optimizer = tf.compat.v1.train.AdamOptimizer(learning_rate=self.lr).minimize(self.exp_cost_final_list[m],
                                                                                       var_list=self.para_exp_list[m])
                rrel_optimizer = tf.compat.v1.train.AdamOptimizer(learning_rate=self.lr / 13.).minimize(self.rrel_cost_final_list[m], 
                                                                                        var_list=self.rpara_rel_list[m])
                rexp_optimizer = tf.compat.v1.train.AdamOptimizer(learning_rate=self.lr / 13.).minimize(self.rexp_cost_final_list[m],
    ...
    def train_model(self, itr, m_list):
        ...
        for m in tqdm(m_list, desc=f"Training sub models", position=0, ascii=True):
            self.generate_R_PR() 
            df = self.df_list[m]
################################################################################################################
