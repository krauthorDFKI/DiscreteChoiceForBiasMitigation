...
class uAE(AbstractRecommender):
    ...
    def create_placeholders(self):
        with tf.name_scope("input_data"):
            ...
            self.tmp1 = tf.compat.v1.placeholder(tf.float32, [None, self.num_items])
            self.tmp2 = tf.compat.v1.placeholder(tf.float32, [None, self.num_items])
            self.tmp3 = tf.compat.v1.placeholder(tf.float32, [None, self.num_items])
