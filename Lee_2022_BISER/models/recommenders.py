...
class PointwiseRecommender(AbstractRecommender):
    ...
    def create_placeholders(self) -> None:
        """Create the placeholders to be used."""
        self.users = tf.compat.v1.placeholder(tf.int32, [None], name='user_placeholder')
        self.items = tf.compat.v1.placeholder(tf.int32, [None], name='item_placeholder')
        ...
        self.tmp1 = tf.compat.v1.placeholder(
            tf.float32, [None, 1], name='tmp1')
        self.tmp2 = tf.compat.v1.placeholder(
            tf.float32, [None, 1], name='tmp2')
        self.tmp3 = tf.compat.v1.placeholder(
    ...
    def build_graph(self) -> None:
        ...
        with tf.name_scope('embedding_layer'):
            ...
            self.user_embeddings = tf.compat.v1.get_variable(
                ...
                initializer=tf.initializers.GlorotUniform()) # Previously in TF 1.X: tf.contrib.layers.xavier_initializer()
            ...
            self.item_bias = tf.compat.v1.get_variable(
                f'item_bias', shape=[self.num_items, 1],
            ...
            self.i_bias = tf.nn.embedding_lookup(
                self.item_bias, self.items)

        with tf.compat.v1.variable_scope('prediction'):
            self.logits = tf.reduce_sum(
                tf.multiply(self.u_embed, self.i_embed), 1) + tf.squeeze(self.i_bias)
    ...
    def add_optimizer(self) -> None:
        ...
        with tf.name_scope('optimizer'):
            # set Adam Optimizer.
            self.apply_grads = tf.compat.v1.train.AdamOptimizer(
...
class MACR:
    def __init__(self, model_name:str, num_users: np.array, num_items: np.array,
        ...
        pos_item_bias = tf.nn.embedding_lookup(self.weights['item_bias'], self.pos_items)
        neg_item_embedding = tf.nn.embedding_lookup(self.weights['item_embedding'], self.neg_items)
        neg_item_bias = tf.nn.embedding_lookup(self.weights['item_bias'], self.neg_items)
        ...
        # self.batch_ratings = tf.matmul(user_embedding, pos_item_embedding, transpose_a=False, transpose_b = True) + pos_item_bias #prediction, shape(user_embedding) != shape(pos_item_embedding)
        # self.user_const_ratings = self.batch_ratings - tf.matmul(self.const_embedding, pos_item_embedding, transpose_a=False, transpose_b = True) - pos_item_bias   #auto tile
        ...
        # self.user_rand_ratings = self.batch_ratings - tf.matmul(user_rand_embedding, pos_item_embedding, transpose_a=False, transpose_b = True) - pos_item_bias
        # self.item_rand_ratings = self.batch_ratings - tf.matmul(user_embedding, item_rand_embedding, transpose_a=False, transpose_b = True) - pos_item_bias
        ...
        # trainable_v1 = tf.compat.v1.get_collection(tf.compat.v1.GraphKeys.TRAINABLE_VARIABLES, 'parameter')
        # self.opt = tf.compat.v1.train.AdamOptimizer(learning_rate=self.lr).minimize(self.loss, var_list = trainable_v1)
        # two branch
        ...
        # self.sigmoid_yu = tf.squeeze(tf.nn.sigmoid(tf.matmul(self.weights['user_embedding'], self.w_user))) # Not used
        # self.sigmoid_yi = tf.squeeze(tf.nn.sigmoid(tf.matmul(self.weights['item_embedding'], self.w))) # Not used
        # two branch bpr
        ...
        # self.opt_two = tf.compat.v1.train.AdamOptimizer(learning_rate=self.lr).minimize(self.loss_two)
        # two branch bce
        ...
        # self.opt_two_bce = tf.compat.v1.train.AdamOptimizer(learning_rate=self.lr).minimize(self.loss_two_bce)
        # two branch bce user&item
        self.mf_loss_two_bce_both, self.reg_loss_two_bce_both = self.create_bce_loss_two_brach_both(user_embedding, pos_item_embedding, neg_item_embedding, pos_item_bias, neg_item_bias)
        self.loss_two_bce_both = self.mf_loss_two_bce_both + self.reg_loss_two_bce_both
        self.opt_two_bce_both = tf.compat.v1.train.AdamOptimizer(learning_rate=self.lr).minimize(self.loss_two_bce_both)
        # 2-stage training
        ...
        # trainable_v2 = tf.compat.v1.get_collection(tf.compat.v1.GraphKeys.TRAINABLE_VARIABLES, 'const_embedding')
        # self.opt2 = tf.compat.v1.train.AdamOptimizer(learning_rate=self.lr).minimize(self.loss2, var_list = trainable_v2)
        ...
        # self.opt2_bce = tf.compat.v1.train.AdamOptimizer(learning_rate=self.lr).minimize(self.loss2_bce, var_list = trainable_v2)
        # self.opt = tf.train.RMSPropOptimizer(learning_rate = self.lr).minimize(self.loss)
        
        # self.opt3 = tf.compat.v1.train.AdamOptimizer(learning_rate=self.lr).minimize(self.loss2, var_list = [self.weights['user_embedding'],self.weights['item_embedding']])
        # self.opt3_bce = tf.compat.v1.train.AdamOptimizer(learning_rate=self.lr).minimize(self.loss2_bce, var_list = [self.weights['user_embedding'],self.weights['item_embedding']])

        self._statistics_params()

        # self.mf_loss_bce, self.reg_loss_bce = self.create_bce_loss(user_embedding, pos_item_embedding, neg_item_embedding, pos_item_bias, neg_item_bias)
        # self.loss_bce = self.mf_loss_bce + self.reg_loss_bce
        # self.opt_bce = tf.compat.v1.train.AdamOptimizer(learning_rate=self.lr).minimize(self.loss_bce)
        ...
        # self.opt_userc_bce = tf.compat.v1.train.AdamOptimizer(learning_rate=self.lr).minimize(self.loss_userc_bce, var_list = [self.weights['user_c']])
        # self.opt = tf.train.RMSPropOptimizer(learning_rate = self.lr).minimize(self.loss)
    ...
    def init_weights(self):
        ...
        with tf.compat.v1.variable_scope('parameter'):
            ...
            weights['item_bias'] = tf.Variable(tf.zeros([self.n_items, 1]), name = 'item_bias')
        ...
        with tf.compat.v1.variable_scope('const_embedding'):
    ...
    #     maxi = tf.compat.v1.log(tf.nn.sigmoid(pos_scores - neg_scores))
    ...
    #     # second branch
    #     maxi_item = tf.compat.v1.log(tf.nn.sigmoid(self.pos_item_scores - self.neg_item_scores))
    ...
    #     pos_scores = pos_scores*tf.nn.sigmoid(self.pos_item_scores)
    #     neg_scores = neg_scores*tf.nn.sigmoid(self.neg_item_scores)
    #     self.mf_loss_ori = tf.reduce_mean(tf.negative(tf.compat.v1.log(tf.nn.sigmoid(pos_scores)+1e-10))+tf.negative(tf.compat.v1.log(1-tf.nn.sigmoid(neg_scores)+1e-10)))
    #     # second branch
    #     self.mf_loss_item = tf.reduce_mean(tf.negative(tf.compat.v1.log(tf.nn.sigmoid(self.pos_item_scores)+1e-10))+tf.negative(tf.compat.v1.log(1-tf.nn.sigmoid(self.neg_item_scores)+1e-10)))
    #     # unify
    ...
    def create_bce_loss_two_brach_both(self, users, pos_items, neg_items, pos_item_bias, neg_item_bias):
        pos_scores = tf.reduce_sum(tf.multiply(users, pos_items), axis=1) + pos_item_bias  #users, pos_items, neg_items have the same shape
        neg_scores = tf.reduce_sum(tf.multiply(users, neg_items), axis=1) + neg_item_bias
        ...
        neg_items_stop = neg_items
        users_stop = users
        self.pos_item_scores = tf.matmul(pos_items_stop,self.w) + pos_item_bias
        self.neg_item_scores = tf.matmul(neg_items_stop,self.w) + neg_item_bias
        ...
        self.mf_loss_ori = tf.reduce_mean(tf.negative(tf.compat.v1.log(tf.nn.sigmoid(pos_scores)+1e-5))+tf.negative(tf.compat.v1.log(1-tf.nn.sigmoid(neg_scores)+1e-5)))
        # second branch
        self.mf_loss_item = tf.reduce_mean(tf.negative(tf.compat.v1.log(tf.nn.sigmoid(self.pos_item_scores)+1e-5))+tf.negative(tf.compat.v1.log(1-tf.nn.sigmoid(self.neg_item_scores)+1e-5)))
        # third branch
        self.mf_loss_user = tf.reduce_mean(tf.negative(tf.compat.v1.log(tf.nn.sigmoid(self.user_scores)+1e-5))+tf.negative(tf.compat.v1.log(1-tf.nn.sigmoid(self.user_scores)+1e-5)))
        # unify
    ...
    # def create_bce_loss(self, users, pos_items, neg_items, pos_item_bias, neg_item_bias):
    #     pos_scores = tf.reduce_sum(tf.multiply(users, pos_items), axis=1) + pos_item_bias  #users, pos_items, neg_items have the same shape
    #     neg_scores = tf.reduce_sum(tf.multiply(users, neg_items), axis=1) + neg_item_bias
    #     # first branch
    #     # fusion
    #     mf_loss = tf.reduce_mean(tf.negative(tf.compat.v1.log(tf.nn.sigmoid(pos_scores)+1e-9))+tf.negative(tf.compat.v1.log(1-tf.nn.sigmoid(neg_scores)+1e-9)))
    #     # regular
    #     regularizer = tf.nn.l2_loss(users) + tf.nn.l2_loss(pos_items) + tf.nn.l2_loss(neg_items)
    #     regularizer = regularizer/self.batch_size
    #     reg_loss = self.decay * regularizer
