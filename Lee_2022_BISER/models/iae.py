...
class iAE(AbstractRecommender):
    ...
    def create_placeholders(self):
        with tf.name_scope("input_data"):
            self.input_R = tf.compat.v1.placeholder(tf.float32, [None, self.num_users])
            self.tmp1 = tf.compat.v1.placeholder(tf.float32, [None, self.num_users])
            self.tmp2 = tf.compat.v1.placeholder(tf.float32, [None, self.num_users])
            self.tmp3 = tf.compat.v1.placeholder(tf.float32, [None, self.num_users])

    def build_graph(self):
        with tf.name_scope("embedding_layer"):  # The embedding initialization is unknown now
            initializer = tf.initializers.GlorotUniform(seed=self.random_state) # Previously in TF 1.X: tf.initializers.GlorotUniform(seed=self.random_state) # Previously in TF 1.X: tf.contrib.layers.xavier_initializer(seed=self.random_state)
    ...
    def add_optimizer(self):
        with tf.name_scope("optimizer"):
            self.apply_grads = tf.compat.v1.train.AdagradOptimizer(learning_rate=self.eta).minimize(self.loss)
