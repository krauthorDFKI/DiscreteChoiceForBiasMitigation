...
from Lee_2022_BISER.evaluate.metrics import average_precision_at_k, dcg_at_k, recall_at_k
...
class PredictRankings:
    ...
    def __init__(self, user_embed: np.ndarray, item_embed: np.ndarray) -> None:
        ...
        self.item_embed = item_embed[0]
        self.item_bias = item_embed[1]
    ...
    def predict(self, users: np.array, items: np.array) -> np.ndarray:
        ...
        if self.item_bias is None:
            ...
            scores = (user_emb @ item_emb.T + item_bias.T
                    ).flatten()
...
class PredictRankings_macr:
    ...
    def __init__(self, user_embed: np.ndarray, item_embed: np.ndarray, pred=None) -> None:
        ...
        if pred is None:
            u_emb_t, i_emb_t, i_bias, w_user, w_item, c = user_embed

            # This is the implementation from the original BISER repo
            # batch_ratings = u_emb_t @ i_emb_t.T + i_bias.T
            # user_scores = u_emb_t @ w_user
            # item_scores = i_emb_t @ w_item
            # pred = batch_ratings *sigmoid(user_scores) 

            # We replaced it to comply with the definition of the MACR inference in the original paper in equation (9) of https://dl.acm.org/doi/pdf/10.1145/3447548.3467289
            batch_ratings = u_emb_t @ i_emb_t.T + i_bias.T
            user_scores = u_emb_t @ w_user
            item_scores = i_emb_t @ w_item + i_bias
            pred = (batch_ratings - c) * sigmoid(item_scores).T * sigmoid(user_scores) 
...
class aoa_evaluator:
    ...
    def __init__(self, user_embed: np.ndarray,
        ...
        if model_name in ['uae']:
            self.model = PredictRankings_AE(weights=user_embed, biases=item_embed, num_users=num_users, num_items=num_items, train=train)
        elif model_name in ['iae']:
            self.model = PredictRankings_iAE(weights=user_embed, biases=item_embed, num_users=num_users, num_items=num_items, train=train)
        elif model_name in ['proposed']:
            self.model = PredictRankings_i_u_AE(weights=user_embed, biases=item_embed, num_users=num_users, num_items=num_items, train=train)
        elif model_name in ['cjmf']:
            self.model = PredictRankings_cjmf(user_embed=user_embed, item_embed=item_embed, num_users=num_users, num_items=num_items)
        elif model_name in ['macr']:
            self.model = PredictRankings_macr(user_embed=user_embed, item_embed=item_embed)
        else:
            self.model = PredictRankings(user_embed=user_embed, item_embed=item_embed)

    def predict(self, user, items):
        return self.model.predict(users=user, items=items)
...
def unbiased_evaluator(user_embed: np.ndarray, 
    ...
    unique_items = np.asarray( range( 50 ) ) # only Set B here
    # calculate ranking metrics

    for user in set(users):
        indices = users == user
        all_pos_items = positive_pairs[positive_pairs[:, 0] == user, 1] 
        # pos_items = items[indices] # Original implementation did this, but also includes negative interactions
        pos_items = np.intersect1d(all_pos_items,  items[indices]) # Seems like they wanted to use only the pos items from the test set, but used all instead. This line fixes that
