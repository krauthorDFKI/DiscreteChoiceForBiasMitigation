...
import numpy as np
from Wan_2022_CPR.tools.io import load_csv_2dict
from Wan_2022_CPR.tools.dataformat import invert_dict
from Wan_2022_CPR.tools.monitor import Timer
...
class Dataset(object):
    def __init__(self, train, n_i_group, n_u_group): #args, data_dir):
        from Lee_2022_BISER.CPR_PD_PointWise_trainer import tocsr, csr_to_user_dict
        self.n_i_group = n_i_group
        self.n_u_group = n_u_group
        ...
        self.train = csr_to_user_dict(tocsr(train_pos, num_users, num_items))
        ...
        if self.n_u_group > 1: # Only used for displaying the results in the paper in a more readable way, but they do not affect the performance
        ...
        if self.n_i_group > 1: # Only used for displaying the results in the paper in a more readable way, but they do not affect the performance
